...
const TagItemMini = ({ tag, selected = false }) => {
  return (
    <Link
      key={tag}
      href={selected ? '/' : `/tag/${encodeURIComponent((tag.name?tag.name:tag))}`}
      passHref
      ...
      <div className='font-light dark:text-gray-400'>{selected && <i className='mr-1 fas fa-tag'/>} {(tag.name?tag.name:tag) + (tag.count ? `(${tag.count})` : '')} </div>
